...
class DemoConfig:
    ...
    model_path: str = "./models_balanced/sign_classifier_final_enhanced.pth"
    ...
    model_search_paths: List[str] = field(default_factory=lambda: [
        "./models_balanced/sign_classifier_final_enhanced.pth",
    ])
...
class SignNetModel:
    ...
    def _detect_architecture(self, state_dict: dict) -> dict:
        ...
        # Detect hidden_size from input_proj
        if 'input_proj.weight' in state_dict:
            arch['hidden_size'] = state_dict['input_proj.weight'].shape[0]

        # Detect num_layers by counting transformer layers
        layer_indices = set()
        ...
        # Detect dim_feedforward from linear1
        if 'transformer_encoder.layers.0.linear1.weight' in state_dict:
            arch['dim_feedforward'] = state_dict['transformer_encoder.layers.0.linear1.weight'].shape[0]

        # Detect num_heads from attention weight shape
        # in_proj_weight shape is [3 * hidden_size, hidden_size]
        if 'transformer_encoder.layers.0.self_attn.in_proj_weight' in state_dict:
            ...
            # Typical head dims: 32, 64, 128
            for num_heads in [16, 12, 8, 6, 4, 2]:
                if arch['hidden_size'] % num_heads == 0:
    ...
    def load(self, model_path: str, vocab_path: str) -> bool:
        """Load model and vocabulary with auto-detected architecture."""
        try:
            ...
            arch = self._detect_architecture(new_state_dict)
            print(f"   Auto-detected architecture:")
            print(f"     hidden_size: {arch['hidden_size']}")
            print(f"     num_layers: {arch['num_layers']}")
            print(f"     num_heads: {arch['num_heads']}")
            print(f"     dim_feedforward: {arch['dim_feedforward']}")
            ...
            if fc_sign_weight is not None:
                ...
                print(f"   Could not detect num_classes from model, using vocab: {num_classes_from_model}")
